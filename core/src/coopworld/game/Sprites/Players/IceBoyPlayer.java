...
/*
 * Class Name: Ice_Boy_Player
 * Description: extends player represents ice boy player
 */
public class IceBoyPlayer extends Player {
    /**
     ...
     *
     */
    public IceBoyPlayer(World w, Screen screen, GeneralUtils utils) {
        super(utils);
        Init(w,screen);
    }

    public void Init(World w, Screen screen){
        textureAtlasString = "players_animations/virtual/boy/boy_pack_combined.pack";
        /* Open pack image og ice boy player. */
       ...
       /* Add frames of jump. */
        ...
        /* Add frames of walk. */
        for(int i = 0; i < 6; i++){
            frames.add(new TextureRegion(this.atlas.findRegion("Boy_Walk"), i * 130, 0, 130, 192));
        }
       /* Pass amount of time between each frame. */
